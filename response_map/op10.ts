...
export default function(information: Buffer, informationlen ) {
    ...
    const pay_direction = (information.slice(pin, (pin+1))).equals(Buffer.from([0x01])) ? 'Income' : 'Expense';
    pin += 1;

    const total_replace_money = hextofloat(information.slice(pin, (pin+4)));
    pin += 4;
    ...
    return {
        ...
        total_replace_money: total_replace_money,
